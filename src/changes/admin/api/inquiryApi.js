...
const API_ADMIN_DETAIL = '/api/admin/inquiry/detail'; // Assuming detail endpoint
const API_ADMIN_REPLY = '/api/admin/inquiry/reply';
...
export async function fetchInquiryDetail(adminLevel, inquiryId) {
  const res = await apiClient.get(`${API_ADMIN_DETAIL}/${inquiryId}`, {
...
export async function writeAdminReply(adminLevel, inquiryId, replyContent) {
  const res = await apiClient.post(API_ADMIN_REPLY, {
...
export async function deleteInquiry(adminLevel, inquiryId) {
  const res = await apiClient.delete(`${API_ADMIN_DELETE}/${inquiryId}`, {
    params: { admin_level: adminLevel }
  });
  return res.data;
}
